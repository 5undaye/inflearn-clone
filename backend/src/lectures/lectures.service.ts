...
export class LecturesService {
    ...
    async create(sectionId: string, createLectureDto: CreateLectureDto, userId: string) {
        ...
        if (section.course.instructorId !== userId) {
            throw new UnauthorizedException('이 섹션에 강의를 추가할 권한이 없습니다.');
        }
    ...
    async update(lectureId: string, updateLectureDto: UpdateLectureDto, userId: string) {
